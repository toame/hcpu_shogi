...
void UCTSearcher::NextStep()
{
	...
	if (playout >= int(playouts_level[pattern][pos_id] * strength + root_node->child_num) && best_move10 == Move::moveNone()) {
		...
		if (max_ >= 2) {
			...
			if (ply > RANDOM_MOVE && pos_root->turn() == Black && abs(sorted_uct_childs[0]->win / sorted_uct_childs[0]->move_count - 0.5f) < 0.42)
			...
			for (int i = 0; i < probabilities.size(); i++) {
				...
				if (grp->group_id == 0 && id < 8) {
					...
					const float rate = select_sum[pattern][pos_id][i] > 0 ? (static_cast<float>(select_count[pattern][pos_id][i]) / select_sum[pattern][pos_id][i]) : 0.0f;
